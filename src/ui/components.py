...
def render_header():
    """Affiche le header avec le widget météo intégré nativement."""
    ...
    if weather["success"]:
        ...
        weather_display = f"{icon} {temp}°C"
    else:
        print(f"Weather Error: {weather.get('error')}")
        weather_display = "🌍 --°C"

    # Utilisation de colonnes natives pour garantir l'affichage
    # Mobile first : Streamlit empile les colonnes sur petit écran
    c_title, c_weather = st.columns([3, 1])

    with c_title:
        st.markdown('<h1 style="text-align: left; margin-bottom: 0;">Gaskiyar Kaya 🇳🇪</h1>', unsafe_allow_html=True)
        st.markdown("<p style='color:#6b7280; font-weight:500; font-size: 1.1em; margin-top: -10px;'>L'Expert Meuble de confiance</p>", unsafe_allow_html=True)

    with c_weather:
        # Affichage métrique native ou markdown simple aligné à droite
        st.markdown(f"""
        <div style='text-align: right; font-size: 0.9em; font-weight: 600; color: var(--text-color); opacity: 0.8;'>
            <div style='margin-bottom: 4px;'>🇳🇪 Niamey</div>
